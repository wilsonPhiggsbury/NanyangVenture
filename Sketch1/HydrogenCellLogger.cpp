...
HydrogenCellLogger::HydrogenCellLogger(HardwareSerial port, Timer *event) :port(&port),eventTimer(event)
{

	Serial.println("Hi from FUELCELL");
}
void HydrogenCellLogger::init()
{
	int id = eventTimer->every(100, &logData);
	Serial.println("Event ID: " + id);
	//if (!(*port))port->begin(9600); // do not re-open port if already open
}
float HydrogenCellLogger::getAmps()
{
	return amps;
}
float HydrogenCellLogger::getVolts()
{
	return volts;
}
float HydrogenCellLogger::getWatts()
{
	return watts;
}
float HydrogenCellLogger::getEnergy()
{
	return energy;
}
static void logData()
