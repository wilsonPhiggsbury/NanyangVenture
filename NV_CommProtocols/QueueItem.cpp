...
bool QueueItem::toQueueItem(char* str, QueueItem* queueItem)
{
	...
	queueItem->ID = (enum DataSource)j;
...
void QueueItem::toFrames(CanFrames* putHere)
{
	uint8_t dataPoints = DATAPOINT_INSTANCES[ID];
	uint8_t readValues = DATAPOINT_READVALUES[ID];
	...
	byte* framePayload = (byte*)&timeStamp;		// Dangerous cast to fit into addItem() function below. Is safeguarded by frameLength
